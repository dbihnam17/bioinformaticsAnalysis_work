## This is a customized version of the oncoplot function in maftools that
## allows for percentages to be displayed with decimal places, and not rounded to
## the nearest whole number

## The rest of this code is not my own and is original to the maftools library

oncoplot_custom <- function (maf, top = 20, minMut = NULL, genes = NULL, altered = FALSE, 
    drawRowBar = TRUE, drawColBar = TRUE, leftBarData = NULL, 
    leftBarLims = NULL, leftBarVline = NULL, leftBarVlineCol = "gray70", 
    rightBarData = NULL, rightBarLims = NULL, rightBarVline = NULL, 
    rightBarVlineCol = "gray70", topBarData = NULL, topBarLims = NULL, 
    topBarHline = NULL, topBarHlineCol = "gray70", logColBar = FALSE, 
    includeColBarCN = TRUE, clinicalFeatures = NULL, annotationColor = NULL, 
    annotationDat = NULL, pathways = NULL, topPathways = 3, path_order = NULL, 
    selectedPathways = NULL, collapsePathway = FALSE, pwLineCol = "#535c68", 
    pwLineWd = 1, draw_titv = FALSE, titv_col = NULL, showTumorSampleBarcodes = FALSE, 
    tsbToPIDs = NULL, barcode_mar = 4, barcodeSrt = 90, gene_mar = 5, 
    anno_height = 1, legend_height = 4, sortByAnnotation = FALSE, 
    groupAnnotationBySize = TRUE, annotationOrder = NULL, sortByMutation = FALSE, 
    keepGeneOrder = FALSE, GeneOrderSort = TRUE, sampleOrder = NULL, 
    additionalFeature = NULL, additionalFeaturePch = 20, additionalFeatureCol = "gray70", 
    additionalFeatureCex = 0.9, genesToIgnore = NULL, removeNonMutated = FALSE, 
    fill = TRUE, cohortSize = NULL, colors = NULL, cBioPortal = FALSE, 
    bgCol = "#CCCCCC", borderCol = "white", annoBorderCol = NA, 
    numericAnnoCol = NULL, drawBox = FALSE, fontSize = 0.8, SampleNamefontSize = 1, 
    titleFontSize = 1.5, legendFontSize = 1.2, annotationFontSize = 1.2, 
    sepwd_genes = 0.5, sepwd_samples = 0.25, writeMatrix = FALSE, 
    colbar_pathway = FALSE, showTitle = TRUE, titleText = NULL, 
    showPct = TRUE) 
{
    ...
    if (!is.null(genes)) {
        ...
        om = maftools:::createOncoMatrix(m = maf, g = genes, add_missing = fill, 
            cbio = cBioPortal)
        ...
        if (is(pathways, "data.frame")) {
            ...
            pathways = match.arg(arg = pathways, choices = c("sigpw", 
                "smgbp"))
            pathwayLoad = get_pw_summary(maf = maf, pathways = pathways)
            if (pathways == "sigpw") {
                pathdb <- system.file("extdata", "oncogenic_sig_patwhays.tsv", 
                  package = "maftools")
            }
            else {
                pathdb <- system.file("extdata", "BP_SMGs.txt.gz", 
                  package = "maftools")
            }
            ...
            if (is.null(selectedPathways)) {
                ...
                if (ncol(pathwayLoad) >= topPathways) {
                  pathways = pathways[Pathway %in% pathwayLoad[1:topPathways, 
                    Pathway]]
                }
                else {
    ...
    if (!removeNonMutated) {
        tsb.include = matrix(data = 0, nrow = nrow(numMat), ncol = length(tsbs[!tsbs %in% 
            colnames(numMat)]))
    ...
    percent_alt = paste0(format(round(100 * (apply(numMat, 1, function(x) length(x[x != 0])) / totSamps), 2), nsmall = 2), "%")
    ...
    if (cBioPortal) {
        if (is.null(colors)) {
            vc_col = grDevices::adjustcolor(col = c("black", 
                "#33A02C", "brown"), alpha.f = 0.7)
            vc_col = c(Truncating = vc_col[1], Missense = vc_col[2], 
                `In-frame` = vc_col[3], Multi_Hit = "#9b59b6", 
                pathway = "#d35400")
        }
        else {
            vc_col = colors
        ...
        if (is.null(colors)) {
            vc_col = get("get_vcColors", envir = asNamespace("maftools"))(alpha = 1, websafe = FALSE, named = TRUE)
        }
        else {
            vc_col = colors
            if (!"pathway" %in% names(vc_col)) {
    ...
    maftools:::plot_layout(clinicalFeatures = clinicalFeatures, drawRowBar = drawRowBar, 
        anno_height = anno_height, drawColBar = drawColBar, draw_titv = draw_titv, 
        exprsTbl = leftBarData, legend_height = legend_height)
    leftBarTitle = NULL
    if (!is.null(leftBarData)) {
        ...
        missing_samps = rownames(numMat)[!rownames(numMat) %in% 
            rownames(leftBarData)]
        if (length(missing_samps) > 0) {
            temp_data = data.frame(row.names = missing_samps, 
                genes = missing_samps, exprn = rep(0, length(missing_samps)), 
                stringsAsFactors = FALSE)
            leftBarData = rbind(leftBarData, temp_data)
        }
        leftBarData = leftBarData[rownames(numMat), , drop = FALSE]
        if (is.null(leftBarLims)) {
            exprs_bar_lims = round(range(leftBarData$exprn, na.rm = TRUE), 
                digits = 2)
        }
        else {
        ...
        if (drawColBar) {
            ...
            plot(x = NA, y = NA, type = "n", axes = FALSE, xlim = exprs_bar_lims, 
                ylim = c(0, 1), xaxs = "i")
        }
    }
    if (drawColBar & is.null(topBarData)) {
        ...
        if (is.null(topBarLims)) {
            topBarLims = round(c(0, max(colSums(x = top_bar_data, 
                na.rm = TRUE))), digits = 2)
        }
        plot(x = NA, y = NA, type = "n", xlim = c(0, ncol(top_bar_data)), 
            ylim = topBarLims, axes = FALSE, frame.plot = FALSE, 
            xlab = NA, ylab = NA, xaxs = "i")
        axis(side = 2, at = c(0, max(topBarLims, na.rm = TRUE)), 
            las = 2, line = 0.5)
        for (i in 1:ncol(top_bar_data)) {
            ...
            x = x[!x == 0]
            if (length(x) > 0) {
                rect(xleft = i - 1, ybottom = c(0, cumsum(x)[1:(length(x) - 
                  1)]), xright = i - 0.1, ytop = cumsum(x), col = vc_col[names(x)], 
                  border = NA, lwd = 0, xpd = FALSE)
            }
        }
        if (logColBar) {
        ...
        if (!is.null(topBarHline)) {
            abline(h = topBarHline, lty = 2, col = topBarHlineCol, 
                xpd = FALSE)
            mtext(text = topBarHline, side = 2, las = 2, cex = 0.6, 
                line = 0.7)
        }
    }
        ...
        if (is.data.frame(topBarData)) {
            ...
            message("Top barplot data is derived from clinical column: ", 
                topBarData)
            extdata <- data.frame(maf@clinical.data)[, c("Tumor_Sample_Barcode", 
                topBarData)]
            rownames(extdata) <- extdata$Tumor_Sample_Barcode
        }
        topbar_title = colnames(extdata)[2]
        colnames(extdata)[2] = "barheights"
        extdata[, "barheights"] <- as.numeric(as.character(extdata[, 
            "barheights"]))
        if (!all(colnames(top_bar_data) %in% rownames(extdata))) {
            missing_topbars = colnames(top_bar_data)[!colnames(top_bar_data) %in% 
                rownames(extdata)]
            warning("Missing column barplot data for ", length(missing_topbars), 
                " samples")
            extdata = rbind(extdata, data.frame(row.names = missing_topbars, 
                Tumor_Sample_Barcode = missing_topbars, barheights = 0, 
                stringsAsFactors = FALSE))
        }
        extdata <- extdata[colnames(top_bar_data), ]
        if (is.null(topBarLims)) {
            topBarLims = round(range(extdata[, "barheights"], 
                na.rm = TRUE), digits = 2)
        }
        plot(x = NA, y = NA, type = "n", xlim = c(0, nrow(extdata)), 
            ylim = topBarLims, axes = FALSE, frame.plot = FALSE, 
            xlab = NA, ylab = NA, xaxs = "i")
        axis(side = 2, at = topBarLims, las = 2, line = 0.5)
        for (i in 1:nrow(extdata)) {
            graphics::rect(xleft = i - 1, xright = i - 0.1, ybottom = 0, 
                ytop = as.numeric(extdata[i, "barheights"]), 
                col = "#535c68", border = NA, lwd = 0, xpd = FALSE)
        }
        title(ylab = topbar_title, font = 1, cex.lab = legendFontSize, 
            xpd = TRUE)
        if (!is.null(topBarHline)) {
            abline(h = topBarHline, lty = 2, col = topBarHlineCol)
        }
    ...
    if (drawRowBar) {
        if (is.null(rightBarData)) {
            side_bar_lims = c(0, max(unlist(apply(numMat, 1, 
                function(x) cumsum(table(x[x != 0])))), na.rm = TRUE))
        }
        else {
            ...
            if (is.null(rightBarLims)) {
                side_bar_lims = round(range(rightBarData$exprn, 
                  na.rm = TRUE), digits = 2)
            }
            else {
        ...
        if (drawColBar) {
            par(mar = c(0.25, 0, 1, 2), xpd = TRUE)
            plot(x = NA, y = NA, type = "n", axes = FALSE, xlim = side_bar_lims, 
                ylim = c(0, 1), xaxs = "i")
    ...
    if (!is.null(leftBarData)) {
        ...
        plot(x = NA, y = NA, type = "n", xlim = rev(-exprs_bar_lims), 
            ylim = c(0, nrow(leftBarData)), axes = FALSE, frame.plot = FALSE, 
        ...
        axis(side = 3, at = rev(-exprs_bar_lims), outer = FALSE, 
            line = 0.25, labels = rev(exprs_bar_lims))
        mtext(text = leftBarTitle, side = 3, line = 0.5, cex = 0.6)
        if (!is.null(leftBarVline)) {
            abline(h = leftBarVline, lty = 2, col = leftBarVlineCol, 
                xpd = FALSE)
        }
    }
    if (showTumorSampleBarcodes) {
    ...
    nm[nm == 0] = NA
    image(x = 1:nrow(nm), y = 1:ncol(nm), z = nm, axes = FALSE, 
    ...
    for (i in 2:length(names(vc_codes_temp))) {
        ...
        suppressWarnings(image(x = 1:nrow(nm), y = 1:ncol(nm), 
            z = nm, axes = FALSE, xaxt = "n", yaxt = "n", xlab = "", 
            ylab = "", col = col, add = TRUE))
    }
    nm = t(apply(numMat, 2, rev))
    nm[nm != 0] = NA
    image(x = 1:nrow(nm), y = 1:ncol(nm), z = nm, axes = FALSE, 
        xaxt = "n", yaxt = "n", xlab = "", ylab = "", col = bgCol, 
        add = TRUE)
    mat_origin = mat_origin[rownames(numMat), colnames(numMat), 
        drop = FALSE]
    if (writeMatrix) {
        write.table(mat_origin, "onco_matrix.txt", sep = "\t", 
            quote = FALSE)
    }
    ...
    if (length(complex_events) > 0) {
        for (i in 1:length(complex_events)) {
            ...
            nm_temp[ce_idx] = 0
            image(x = 1:nrow(nm_temp), y = 1:ncol(nm_temp), z = nm_temp, 
                axes = FALSE, xaxt = "n", yaxt = "n", xlab = "", 
                ylab = "", col = vc_col[ce[2]], add = TRUE)
            ce_idx = which(t(nm_temp) == 0, arr.ind = TRUE)
            for (i in seq_len(nrow(ce_idx))) {
                rowi = ce_idx[i, 1]
                coli = ce_idx[i, 2]
                rect(xleft = coli - 0.5, ybottom = rowi - 0.25, 
                  xright = coli + 0.5, ytop = rowi + 0.25, col = vc_col[ce[1]], 
                  border = NA, lwd = 0)
            }
        }
    }
    for (cnevent in om$cnvc) {
        ...
        if (nrow(cn_idx) > 0) {
            ...
            image(x = 1:nrow(nm_temp), y = 1:ncol(nm_temp), z = nm_temp, 
                axes = FALSE, xaxt = "n", yaxt = "n", xlab = "", 
                ylab = "", col = bgCol, add = TRUE)
            cn_idx = which(t(nm_temp) == 0, arr.ind = TRUE)
            for (i in seq_len(nrow(cn_idx))) {
                rowi = cn_idx[i, 1]
                coli = cn_idx[i, 2]
                rect(xleft = coli - 0.5, ybottom = rowi - 0.25, 
                  xright = coli + 0.5, ytop = rowi + 0.25, col = vc_col[cnevent], 
                  border = NA, lwd = 0)
            }
        }
    }
    ...
    if (!is.null(pathways)) {
        temp_dat = lapply(seq_along(temp_dat), function(i) {
            data.table::rbindlist(list(temp_dat[[i]], data.table::data.table(Gene = names(temp_dat)[i])), 
                use.names = TRUE, fill = TRUE)
        })
        temp_dat = data.table::rbindlist(l = temp_dat, use.names = TRUE, 
            fill = TRUE)
        temp_dat[, `:=`(row_id, nrow(temp_dat):1)]
        temp_dat = split(temp_dat, as.factor(temp_dat$Pathway))
        if (!collapsePathway) {
            lapply(temp_dat, function(td) {
                rect(xleft = 0.5, ybottom = min(td$row_id) - 
                  0.499, xright = nrow(nm) + 0.5, ytop = max(td$row_id) + 
                  0.499, border = pwLineCol, lwd = pwLineWd)
            })
        }
    ...
    if (!is.null(additionalFeature)) {
        if (is.list(additionalFeature)) {
            for (af_idx in seq_along(additionalFeature)) {
                ...
                af_dat = subsetMaf(maf = maf, genes = rownames(numMat), 
                  tsb = colnames(numMat), fields = af[1], includeSyn = FALSE, 
                  mafObj = FALSE)
                if (length(which(colnames(af_dat) == af[1])) == 
                  0) {
                ...
                if (nrow(af_dat) == 0) {
                  warning(paste0("No samples are enriched for ", 
                    af[2], " in ", af[1]))
                }
                else {
                  af_mat = data.table::dcast(data = af_dat, Tumor_Sample_Barcode ~ 
                    Hugo_Symbol, value.var = "temp_af", fun.aggregate = length)
                  ...
                  if (length(additionalFeaturePch) != length(additionalFeature)) {
                    ...
                    additionalFeaturePch = rep(additionalFeaturePch, 
                      length(additionalFeature))
                  }
                  if (length(additionalFeatureCol) != length(additionalFeature)) {
                    warning("Provided colors for additional features are recycled")
                    additionalFeatureCol = rep(additionalFeatureCol, 
                      length(additionalFeature))
                  }
                  lapply(seq_len(nrow(af_mat)), function(i) {
                    af_i = af_mat[i, , drop = FALSE]
                    af_i_genes = colnames(af_i)[which(af_i > 
                      0)]
                    af_i_sample = rownames(af_i)
                    lapply(af_i_genes, function(ig) {
                      af_i_mat = matrix(c(which(rownames(nm) == 
                        af_i_sample), which(colnames(nm) == ig)), 
                        nrow = 1)
                      points(af_i_mat, pch = additionalFeaturePch[af_idx], 
                        col = additionalFeatureCol[af_idx], cex = additionalFeatureCex)
                    })
                  })
            ...
            af_dat = subsetMaf(maf = maf, genes = rownames(numMat), 
                tsb = colnames(numMat), fields = additionalFeature[1], 
                includeSyn = FALSE, mafObj = FALSE)
            if (length(which(colnames(af_dat) == additionalFeature[1])) == 
                0) {
                message(paste0("Column ", additionalFeature[1], 
                  " not found in maf. Here are available fields.."))
                print(getFields(maf))
                stop()
            }
            ...
            if (nrow(af_dat) == 0) {
                warning(paste0("No samples are enriched for ", 
                  additionalFeature[2], " in ", additionalFeature[1]))
            }
            else {
                af_mat = data.table::dcast(data = af_dat, Tumor_Sample_Barcode ~ 
                  Hugo_Symbol, value.var = "temp_af", fun.aggregate = length)
                ...
                lapply(seq_len(nrow(af_mat)), function(i) {
                  ...
                  lapply(af_i_genes, function(ig) {
                    af_i_mat = matrix(c(which(rownames(nm) == 
                      af_i_sample), which(colnames(nm) == ig)), 
                      nrow = 1)
                    points(af_i_mat, pch = additionalFeaturePch, 
                      col = additionalFeatureCol, cex = additionalFeatureCex)
                  })
                })
                additionalFeature_legend = TRUE
            }
    ...
    mtext(text = colnames(nm), side = 2, at = 1:ncol(nm), font = 3, 
    ...
    if (showTumorSampleBarcodes) {
        if (!is.null(tsbToPIDs)) {
            if (is.data.frame(x = tsbToPIDs)) {
                colnames(x = tsbToPIDs) = c("Tumor_Sample_Barcode", 
                  "PID")
                ...
                tsbToPIDs = clin[, c("Tumor_Sample_Barcode", 
                  tsbToPIDs)]
                colnames(x = tsbToPIDs) = c("Tumor_Sample_Barcode", 
                  "PID")
            }
            if (all(rownames(nm) %in% rownames(tsbToPIDs))) {
                text(x = 1:nrow(nm), y = par("usr")[3] - 0.2, 
                  labels = tsbToPIDs[rownames(nm), 2], srt = barcodeSrt, 
                  font = 1, cex = SampleNamefontSize, adj = 1)
            }
            else {
                missing_tsb = setdiff(rownames(nm), rownames(tsbToPIDs))
                warning("Follwing Tumor_Sample_Barcodes are missing corresponding PIDs: ", 
                  paste(missing_tsb, collapse = ", "))
                text(x = 1:nrow(nm), y = par("usr")[3] - 0.2, 
                  labels = tsbToPIDs[rownames(nm), 2], srt = barcodeSrt, 
                  font = 1, cex = SampleNamefontSize, adj = 1)
            }
            ...
            text(x = 1:nrow(nm), y = par("usr")[3] - 0.2, labels = rownames(nm), 
                srt = barcodeSrt, font = 1, cex = SampleNamefontSize, 
                adj = 1)
        }
    }
    if (drawRowBar) {
        ...
        if (is.null(rightBarData)) {
            ...
            plot(x = NA, y = NA, type = "n", xlim = side_bar_lims, 
                ylim = c(0, length(side_bar_data)), axes = FALSE, 
                frame.plot = FALSE, xlab = NA, ylab = NA, xaxs = "i", 
                yaxs = "i")
            for (i in 1:length(side_bar_data)) {
                x = rev(side_bar_data)[[i]]
                if (length(x) > 0) {
                  rect(ybottom = i - 1, xleft = c(0, cumsum(x)[1:(length(x) - 
                    1)]), ytop = i - 0.1, xright = cumsum(x), 
                    col = vc_col[vc_codes[names(x)]], border = NA, 
                    lwd = 0)
                }
            }
            if (!is.null(rightBarVline)) {
                abline(v = rightBarVline, lty = 2, col = rightBarVlineCol, 
                  xpd = FALSE)
            }
            axis(side = 3, at = side_bar_lims, outer = FALSE, 
                line = 0.25)
            mtext(text = rightBarTitle, side = 3, line = 0.5, 
                cex = 0.6)
        }
        else {
            plot(x = NA, y = NA, type = "n", xlim = side_bar_lims, 
                ylim = c(0, nrow(rightBarData)), axes = FALSE, 
                frame.plot = FALSE, xlab = NA, ylab = NA, xaxs = "i", 
                yaxs = "i")
    ...
    if (!is.null(clinicalFeatures)) {
        ...
        clini_lvls = lapply(clini_lvls, function(cl) {
            temp_names = suppressWarnings(sample(x = setdiff(x = 1:1000, 
                y = as.numeric(as.character(cl))), size = length(cl), 
                replace = FALSE))
            names(cl) = temp_names
            cl
        })
        ...
        annotation_color_coded = data.frame(row.names = rownames(annotation), 
            stringsAsFactors = FALSE)
        for (i in 1:length(clini_lvls)) {
            cl = clini_lvls[[i]]
            clname = names(clini_lvls)[i]
            cl_vals = annotation[, clname]
            for (i in 1:length(cl)) {
                cl_vals[cl_vals == cl[i]] = names(cl[i])
            }
            annotation_color_coded = cbind(annotation_color_coded, 
                cl_vals, stringsAsFactors = FALSE)
        }
        names(annotation_color_coded) = names(clini_lvls)
        annotation = data.frame(lapply(annotation_color_coded, 
            as.numeric), stringsAsFactors = FALSE, row.names = temp_rownames)
        annotation = annotation[colnames(numMat), ncol(annotation):1, 
            drop = FALSE]
        if (!is.null(leftBarData)) {
            plot.new()
        }
        ...
        image(x = 1:nrow(annotation), y = 1:ncol(annotation), 
            z = as.matrix(annotation), axes = FALSE, xaxt = "n", 
            yaxt = "n", bty = "n", xlab = "", ylab = "", col = "white")
        for (i in 1:length(clini_lvls)) {
            cl = clini_lvls[[i]]
            ...
            for (i in 1:length(names(cl))) {
                ...
                suppressWarnings(image(x = 1:nrow(temp_anno), 
                  y = 1:ncol(temp_anno), z = temp_anno, axes = FALSE, 
                  xaxt = "n", yaxt = "n", xlab = "", ylab = "", 
                  col = col, add = TRUE))
            }
        }
        abline(h = (0:(ncol(nm))) + 0.5, col = annoBorderCol, 
            lwd = sepwd_genes)
        abline(v = (0:(nrow(nm))) + 0.5, col = annoBorderCol, 
            lwd = sepwd_samples)
        mtext(text = colnames(annotation), side = 4, font = 1, 
    ...
    if (draw_titv) {
        ...
        missing_samps = colnames(numMat)[!colnames(numMat) %in% 
            colnames(titv_dat)]
        if (length(missing_samps) > 0) {
        ...
        plot(x = NA, y = NA, type = "n", xlim = c(0, ncol(titv_dat)), 
            ylim = c(0, 100), axes = FALSE, frame.plot = FALSE, 
            xlab = NA, ylab = NA, xaxs = "i")
        if (is.null(titv_col)) {
        ...
        for (i in 1:ncol(titv_dat)) {
            ...
            if (length(x) > 0) {
                rect(xleft = i - 1, ybottom = c(0, cumsum(x)[1:(length(x) - 
                  1)]), xright = i - 0.1, ytop = cumsum(x), col = titv_col[names(x)], 
                  border = NA, lwd = 0)
            }
            else {
                rect(xleft = i - 1, ybottom = c(0, 100), xright = i - 
                  0.1, ytop = 100, col = "gray70", border = NA, 
                  lwd = 0)
            }
        }
        if (drawRowBar) {
            ...
            lep = legend("topleft", legend = names(titv_col), 
                col = titv_col, border = NA, bty = "n", ncol = 2, 
                pch = 15, xpd = TRUE, xjust = 0, yjust = 0, cex = legendFontSize)
        }
        else {
            mtext(text = names(titv_col)[1:3], side = 2, at = c(25, 
                50, 75), font = 1, line = 0.4, cex = fontSize, 
                las = 2, col = titv_col[1:3])
            mtext(text = names(titv_col)[4:6], side = 4, at = c(25, 
                50, 75), font = 1, line = 0.4, cex = fontSize, 
                las = 2, col = titv_col[4:6], adj = 0.15)
        }
    }
    ...
    if (additionalFeature_legend) {
        ...
        for (af_idx in 1:length(additionalFeature)) {
            ...
            names(leg_classes)[length(leg_classes)] = paste(af, 
                collapse = ":")
    ...
    lep = legend("topleft", legend = names(leg_classes), col = leg_classes, 
        border = NA, bty = "n", ncol = 2, pch = leg_classes_pch, 
        xpd = TRUE, xjust = 0, yjust = 0, cex = legendFontSize)
    x_axp = 0 + lep$rect$w
    if (!is.null(clinicalFeatures)) {
        ...
        for (i in 1:ncol(annotation)) {
            ...
            if (is_num) {
                ...
                image(y = c(0.1, 0.2), x = seq(num_start, num_start + 
                  num_width, length.out = 200), z = t(matrix(seq(0, 
                  1, length.out = 200), nrow = 1)), col = x, 
                  xlim = c(0, 1), ylim = c(0, 1), axes = FALSE, 
                  xlab = NA, ylab = NA, add = TRUE)
                text(x = c(num_start, num_start + num_width), 
                  y = 0.2, labels = x_range, pos = 3)
                text(x = num_start + (num_width/2), y = 0.1, 
                  labels = clinicalFeatures[i], pos = 1, adj = 1)
                ...
                lep = legend(x = x_axp, y = 1, legend = names(x), 
                  col = x, border = NA, ncol = n_col, pch = 15, 
                  xpd = TRUE, xjust = 0, bty = "n", cex = annotationFontSize, 
                  title = rev(names(annotation))[i], title.adj = 0)
                x_axp = x_axp + lep$rect$w
            }
        }
    }
    if (removeNonMutated) {
        altStat = paste0("Altered in ", ncol(numMat), " (", round(ncol(numMat)/totSamps, 
            digits = 4) * 100, "%) of ", totSamps, " samples.")
    }
    else {
        mutSamples = length(unique(unlist(genesToBarcodes(maf = maf, 
            genes = rownames(numMat), justNames = TRUE, verbose = FALSE))))
    ...
    if (showTitle) {
        if (is.null(titleText)) {
            ...
            title(main = titleText, outer = TRUE, line = -1, 
                cex.main = titleFontSize)
        }
    }
